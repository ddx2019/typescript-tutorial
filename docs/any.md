...
```typescript
//当  "noImplicitAny": true时，
let x;//不报错，因为 x类型被推断为any类型，但x是用let声明的变量，未赋值且未指定类型，是不会报错的。
x.foo=1 // 报错,“x”可能为“undefined”；但是any类型包含了一切可能的类型，那x可能是undefined ，试图从undefined上读取foo属性，则会报编译错
```
